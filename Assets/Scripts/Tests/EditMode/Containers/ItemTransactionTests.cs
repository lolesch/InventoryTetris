...
namespace ToolSmiths.InventorySystem.Tests.EditMode.Containers
{
    ...
    public sealed class ItemTransactionTests
    {
        ...
        public void CursorHolder_OnCommit_HandsTheHeldPackageToTheSink()
        {
            ...
            {
                Assert.That(cursor.TryHold(displaced, new PackageOrigin(inventory, Vector2Int.zero)), Is.True);
        ...
        public void CursorHolder_OnRollback_NeverTouchesTheSink()
        {
            ...
            using (new ItemTransaction(cursor, inventory))
                _ = cursor.TryHold(new Package(inventory, Sword(), 1u), new PackageOrigin(inventory, Vector2Int.zero)); // no commit
        ...
        public void CursorHolder_IsOneCapacity()
        {
            ...
            Assert.That(cursor.TryHold(new Package(inventory, Sword(), 1u), new PackageOrigin(inventory, Vector2Int.zero)), Is.True);
            Assert.That(cursor.TryHold(new Package(inventory, Arrows(), 1u), new PackageOrigin(inventory, Vector2Int.zero)), Is.False);
            Assert.That(cursor.IsFree, Is.False);
        }
        ...
        public void TryReHomeToHandOrContainer_PutsTheDisplacedItemOnTheFreedCursorFirst()
        {
            ...
            {
                Assert.That(transaction.TryReHomeToHandOrContainer(ref displaced, new PackageOrigin(origin, Vector2Int.zero)), Is.True);
        ...
        public void TryReHomeToHandOrContainer_WhenTheCursorIsAlreadyHolding_FallsThroughToTheFirstReHomeContainer()
        {
            ...
            _ = transaction.TryReHomeToHandOrContainer(ref first, new PackageOrigin(origin, Vector2Int.zero)); // takes the cursor

            var second = new Package(origin, Helm(2f), 1u);
            Assert.That(transaction.TryReHomeToHandOrContainer(ref second, new PackageOrigin(origin, Vector2Int.zero)), Is.True);
        ...
        public void TryReHomeToHandOrContainer_WhenNothingCanTakeIt_AbortsTheTransactionAndCommitRollsBack()
        {
            ...
            {
                var first = new Package(origin, Sword(), 1u);
                _ = transaction.TryReHomeToHandOrContainer(ref first, new PackageOrigin(origin, Vector2Int.zero)); // cursor

                var second = new Package(origin, Helm(2f), 1u);
                Assert.That(transaction.TryReHomeToHandOrContainer(ref second, new PackageOrigin(origin, Vector2Int.zero)), Is.False);
        ...
        public void TryReHomeToContainerOrHand_PrefersTheContainer_ThenFallsBackToTheHand()
        {
            ...
            {
                var first = new Package(origin, Sword(), 1u);
                Assert.That(transaction.TryReHomeToContainerOrHand(ref first, new PackageOrigin(origin, Vector2Int.zero)), Is.True);
                Assert.That(origin.StoredPackages.Values.Single().Item.DefinitionId, Is.EqualTo(SwordId), "the first went into the container");

                var second = new Package(origin, Helm(2f), 1u);
                Assert.That(transaction.TryReHomeToContainerOrHand(ref second, new PackageOrigin(origin, Vector2Int.zero)), Is.True);
